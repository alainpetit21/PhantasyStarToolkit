...
import Model.PSRomFile;


public class MessageOnPSRomFileOpened implements Model.IObserverPSRomFileOpened{
    ...
    private void loadPSRomFile(PSRomFile pObjPSRomFile) {
        ...
        for(int i= 0; i < pObjPSRomFile.getNbMonster(); ++i){
            try {
                ...
                Model.ModelFacade.getInstance().addMonster(monsterData);
                
            }catch(Model.PSRomFile.Exception_InvalidateState eis){
                System.out.println("Invalid State of PhantasyStarRomFile");
